...
export const hook = async (options: { command: string, flags: any, config: IConfig }) => {
  ...
  try {
    ...
    if (!options.flags.telemetry && !segmentTelemetry) {
      const confirmed = await cli.confirm(`${getProjectlName()} would like to collect data about how users use cli commands. Participation is voluntary and when you choose to participate crwctl automatically sends statistic usage about how you use the cli [y/n]:`)
