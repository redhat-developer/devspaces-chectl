    ...
    if (ctx.deployedCheOperatorImageName === DEFAULT_CHE_OPERATOR_IMAGE_NAME && ctx.newCheOperatorImageName === DEFAULT_CHE_OPERATOR_IMAGE_NAME) {
      ...
      if (ctx.deployedCheOperatorImage === ctx.newCheOperatorImage) {
        if (ctx.newCheOperatorImageTag === 'nightly') {
          cli.info('Updating current CodeReady Workspaces nightly version to a new one.')
          return true
        }
      ...
      if (this.isUpgrade(ctx.deployedCheOperatorImageTag, ctx.newCheOperatorImageTag)) {
        ...
        if (!ctx.isNightly && (ctx.newCheOperatorImageTag === 'nightly' || semver.lt(currentChectlVersion, ctx.newCheOperatorImageTag))) {
          ...
          if (ctx.newCheOperatorImageTag === 'nightly') {
            cli.warn(`Stable ${getProjectName()} cannot update stable CodeReady Workspaces to nightly version`)
          } else {
        ...
        // Upgrade allowed
        if (ctx.newCheOperatorImageTag === 'nightly') {
          cli.info(`You are going to update CodeReady Workspaces ${ctx.deployedCheOperatorImageTag} to nightly version.`)
        } else {
        ...
        if (semver.gt(MIN_CHE_OPERATOR_INSTALLER_VERSION, flags.version)) {
          cli.info(`Given CodeReady Workspaces version ${flags.version} is too old to be downgraded to`)
          return false
        }

        cli.info(`You are going to downgrade CodeReady Workspaces ${ctx.deployedCheOperatorImageTag} to ${ctx.newCheOperatorImageTag}`)
