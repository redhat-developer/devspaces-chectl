...
export class OperatorInstaller implements Installer {
  ...
  private static readonly WEBHOOK_SERVICE = 'devspaces-operator-service'
  ...
  getDeleteTasks(): Listr.ListrTask<any>[] {
    ...
    return [
      ...
      },
      {
        ...
        task: async (_ctx: any, task: any) => {
          try {
            await this.kh.deleteSecret('devspaces-operator-webhook-server-cert', this.flags.chenamespace)
            task.title = `${task.title}...[Ok]`
          } catch (e: any) {
            task.title = `${task.title}...[Failed: ${e.message}]`
          }
