...
export class OperatorTasks {
  private static readonly WEBHOOK_SERVICE = 'devspaces-operator-webhook-service'
  ...
  getDeleteTasks(flags: any): ReadonlyArray<Listr.ListrTask> {
    ...
    return [
      ...
      },
      {
        ...
        task: async (_ctx: any, task: any) => {
          try {
            await this.kh.deleteSecret('devspaces-operator-webhook-server-cert', flags.chenamespace)
            task.title = `${task.title}...[Ok]`
          } catch (e: any) {
            task.title = `${task.title}...[Failed: ${e.message}]`
          }
