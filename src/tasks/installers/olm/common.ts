...
export function getApproveInstallPlanTask(flags: any): Listr.ListrTask<Listr.ListrContext> {
  ...
  return {
    ...
    task: async (ctx: any, task: any) => {
      const subscription = await cheHelper.findCheOperatorSubscription(OPENSHIFT_OPERATORS_NAMESPACE)
      if (!subscription) {
        throw new Error('Red Hat OpenShift Dev Spaces subscription not found.')
      }
      ...
      if (subscription.status) {
        ...
        if (subscription.status.state === 'UpgradePending') {
          ...
          if (subscription.status.installplan?.name) {
            ...
            throw new Error('Red Hat OpenShift Dev Spaces InstallPlan name is empty.')
          }

          await kubeHelper.approveOperatorInstallationPlan(ctx[OLM.INSTALL_PLAN], OPENSHIFT_OPERATORS_NAMESPACE)
          await kubeHelper.waitOperatorInstallPlan(ctx[OLM.INSTALL_PLAN], OPENSHIFT_OPERATORS_NAMESPACE, 60)
          if (installedCSV) {
            ctx.highlightedMessages.push(`Red Hat OpenShift Dev Spaces Operator is upgraded from '${getVersionFromCSV(installedCSV)}' to '${getVersionFromCSV(currentCSV)}' version`)
          } else {
            ctx.highlightedMessages.push(`Red Hat OpenShift Dev Spaces '${getVersionFromCSV(currentCSV)}' version installed`)
          }
          task.title = `${task.title}...[OK]`
          return
        }

        throw new Error(`Red Hat OpenShift Dev Spaces Subscription in '${subscription.status.state}' state.`)
      }

      throw new Error('Red Hat OpenShift Dev Spaces InstallPlan not found.')
    },
  }
}
  ...
  return {
    ...
    task: async (ctx: any, task: Listr.ListrTaskWrapper<any>) => {
      ...
      if (subscription.spec.installPlanApproval === OLMInstallationUpdate.AUTO) {
        ...
        throw new Error('Use \'dsc server:update\' command only with \'Manual\' InstallPlan approval strategy.')
      }
