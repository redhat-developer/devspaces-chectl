    ...
    return [
      ...
      },
      {
        ...
        task: async (_ctx: any, task: any) => {
          if (await kube.isRoleExist(this.prometheusRoleName, flags.chenamespace)) {
      ...
      },
      {
        ...
        task: async (_ctx: any, task: any) => {
          if (await kube.isRoleBindingExist(this.prometheusRoleBindingName, flags.chenamespace)) {
            task.title = `${task.title}...[Exists]`
          } else {
      ...
      },
      {
        ...
        task: async (_ctx: any, task: any) => {
          if (await che.findCheOperatorOperatorGroup(flags.chenamespace)) {
            task.title = `${task.title}...[Exists]`
          } else {
      ...
      },
      {
        ...
        task: async (ctx: any, task: any) => {
          if (!await kube.IsCatalogSourceExists(NEXT_CATALOG_SOURCE_NAME, ctx.operatorNamespace)) {
      ...
      },
      {
        ...
        task: async (ctx: any, task: any) => {
          ...
          if (!await kube.IsCatalogSourceExists(customCatalogSource.metadata!.name!, flags.chenamespace)) {
