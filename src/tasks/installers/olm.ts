...
export class OLMTasks {
  ...
  startTasks(flags: any, command: Command): Listr {
    ...
    if (this.isNightlyChectlChannel() && !flags['catalog-source-yaml']) {
      command.warn('A nightly channel for CodeReady Workspaces is not available on OpenShift OLM catalog, the latest stable release will be deployed instead. To get a nightly release of CodeReady Workspaces use the `operator` installer (--installer=operator).')
    }
    return new Listr([
